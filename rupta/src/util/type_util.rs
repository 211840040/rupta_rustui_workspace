...
use rustc_middle::ty::{GenericArgKind, GenericArgsRef, TypingEnv, TypingMode};
use rustc_middle::ty::{Const, ExistentialPredicate, FieldDef, ParamEnv, PolyFnSig, Ty, TyCtxt, TyKind};
...
/// caches pointer-type fields for a given rust type
...
impl<'tcx> PointerProjectionsCache<'tcx> {
    ...
    pub fn get_pointer_projections(
        ...
        match self.ptr_projs_cache.entry(base_ty) {
            //return a mutable reference to the value for the key
...
impl<'tcx> FieldByteOffsetCache<'tcx> {
    ...
    struct Inner {
        a: u8,         // offset 0
        b: u32,        // offset 4 (due to alignment)
    }
    ...
    After computing, we have the following mapping:
        Outer: {
            [Field(0)]: 0,
            [Field(0), Field(0)]: 0, // Outer.x.a
            [Field(0), Field(1)]: 4, // Outer.x.b
            [Field(1)]: 8,
        },
        Inner: {
            [Field(0)]: 0,
    ...
    pub fn compute_fields_byte_offsets(&mut self, tcx: TyCtxt<'tcx>, ty: Ty<'tcx>) {
        ...
        /* #079 @lzq */
        let param_env = rustc_middle::ty::TypingEnv::fully_monomorphized().param_env;
        ...
        match ty.kind() {
            TyKind::Adt(adt_def, args) if adt_def.is_struct() => {
                if let Ok(layout) = layout_of(tcx, param_env, ty) {
                    ...
                    if let rustc_abi::FieldsShape::Arbitrary {
                        offsets,
                        memory_index: _,
                    } = layout.fields()
                    {
...
impl<'tcx> PathCastCache<'tcx> {
    ...
    /// rust allow more than one field take the first memory location of a struct
    /// for example:struct S {
    ///     a: Zst,      // offset 0, size 0
    ///     b: u32,      // offset 0, size 4
    ///     c: Zst,      // offset 0, size 0
    /// }
    /// let t: ((), u32, ()) = ((), 42, ());
    /// t.0, t.1, t.2 all point to the same memory location
    pub fn cast_to(
        &mut self,
    ...
    Author: wy
    Date: 2025-10-22
    regularize the given path by removing cast selectors, index selectors, zero-offset field selectors and downcast selectors
     */
        ...
        if let PathEnum::QualifiedPath { base: _, projection } = &path.value {
            match projection.last().unwrap() {
                ...
                PathSelector::Field(f) => {
                    ...
                    // projection has a selector field means the base type is a struct?
...
pub fn is_dynamic_fn_trait<'tcx>(tcx: TyCtxt<'tcx>, ty: Ty<'tcx>) -> bool {
    if let TyKind::Dynamic(trait_data, ..) = ty.kind() {
        if let Some(principal) = trait_data.principal() {
            let principal = tcx.normalize_erasing_late_bound_regions(TypingEnv::fully_monomorphized(), principal);
...
get_pointer_projections(Outer) -> [
    ([Field(0), Field(1)], *const u8),
    ([Field(1)], *mut f64)
]
 */
...
pub fn get_array_length<'tcx>(
    ...
    // Attention: The results may differ due to the lack of consts::kind::prepare_for_eval in nightly-1.77.
    // Try to evaluate the constant using the parameter environment
    // This handles both evaluated constants and unevaluated constants that can be resolved
    if let Some(val) = crate::util::try_eval_target_usize_with_param_env(tcx, param_env, *length) {
        val as usize
    } else {
...
Author: wy
Date: 2025-10-23
/// get the layout(of storage)of the given type
*/
#[inline]
        ...
        TyKind::Adt(adt_def, args) => {
            ...
            if !adt_def.variants().is_empty() {
                ...
                if let Ok(layout) = layout_of(tcx, param_env, path_ty) {
                    let layout = layout.layout;
                    let variant = adt_def.variants().iter().next().expect("at least one variant");
                    if let rustc_abi::FieldsShape::Arbitrary { offsets, memory_index: _ } = layout.fields() {
                        for (field_idx, offset) in offsets.iter_enumerated() {
                            let field = &variant.fields[field_idx];
                            let field_path = Path::new_field(path.clone(), field_idx.index());
                            let field_ty = field_ty(tcx, field, args);
                            let offset = offset.bytes_usize() + base_offset;
                            flatten_fields_recursively(
                                tcx,
                                param_env,
                                field_path,
        ...
        TyKind::Tuple(types) => {
            if let Ok(layout) = layout_of(tcx, param_env, path_ty) {
                ...
                if let rustc_abi::FieldsShape::Arbitrary { offsets, memory_index: _ } = layout.fields() {
                    for (field_idx, offset) in offsets.iter_enumerated() {
                        let field_path = Path::new_field(path.clone(), field_idx.index());
                        let field_ty = types[field_idx.index()];
                        let offset = offset.bytes_usize() + base_offset;
                        flatten_fields_recursively(
                            tcx,
                            param_env,
...
/// Author: wy
/// Date: 2025-10-26
/// find fields at the start memory location of the given type recursively
/// layout.layout.fields() contains of offsets and memory_index
/// we use offsets[index] to confirm whether the field is at the start memory location
/// if offsets[index] == 0, the field is at the start memory location
    ...
    match path_ty.kind() {
        TyKind::Adt(adt_def, args) => {
            ...
            if adt_def.is_union() {
                ...
                // adt_def.variants() return a slice with a single variant for union types
                let variant = adt_def.variants().iter().next().expect("at least one variant");
        ...
        TyKind::Tuple(types) => {
            if let Ok(layout) = layout_of(tcx, param_env, path_ty) {
                let layout = layout.layout;
                if let rustc_abi::FieldsShape::Arbitrary { offsets, memory_index: _ } = layout.fields() {
                    // There may be multiple fields at start memory location as a field can be zero sized.
                    for (field_idx, offset) in offsets.iter_enumerated() {
                        if offset.bytes_usize() == 0 {
                            let field_path = Path::new_field(path.clone(), field_idx.index());
                            let field_ty = types[field_idx.index()];
...
Author:wangyan
Date:2025-10-22
///optimized_mir() -> optimized mir of given defid(functionid)
///evaluate the type of a local decl in a function's mir
 */
...
pub fn eval_local_decl_type<'tcx>(
    ...
    substs_specializer.specialize_generic_argument_type(mir.local_decls[Local::from_usize(ordinal)].ty)
}

/*
Author:wangyan
Date:2025-10-22
/// evaluate the type of a uncached path
/// All auxiliary variables, promoted constants and function paths types should have been cached when creating the paths.
/// cant tell the type of offset path since different qualified paths with different types
 */
            ...
            while !projection.is_empty() {
                ...
                let projection_ty = match projection_elem {
                    ...
                    PathSelector::Downcast(ordinal) => {
                        get_downcast_type(acx.tcx, base_ty, VariantIdx::new(*ordinal))
                    }
