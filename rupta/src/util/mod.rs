...
use rustc_middle::ty::{GenericArgsRef, ParamEnv, TypingEnv, TypingMode, TyCtxt, TyKind};
use std::io::Write;
use std::rc::Rc;
    ...
    generic_args
        ...
        .map(|t| match t.unpack() {
            ...
            rustc_middle::ty::GenericArgKind::Const(c) => {
                if let Some(val) = try_eval_target_usize(tcx, rustc_middle::ty::TypingEnv::fully_monomorphized(), c) {
...
/* #078 @lzq */
/* Possible loss of analysis accuracy, corresponding methods for nightly-05-09 not found: try_eval_target_usize, prepare_for_eval */
#[inline]
fn try_eval_target_usize<'tcx>(
    tcx: TyCtxt<'tcx>,
    typing_env: rustc_middle::ty::TypingEnv<'tcx>,
    c: rustc_middle::ty::Const<'tcx>,
) -> Option<u64> {
    match c.kind() {
        rustc_middle::ty::ConstKind::Value(val) => val.try_to_target_usize(tcx),
        rustc_middle::ty::ConstKind::Unevaluated(uv) => {
            match tcx.const_eval_resolve_for_typeck(typing_env, uv, rustc_span::DUMMY_SP) {
                Ok(Ok(valtree)) => Some(valtree.try_to_scalar_int()?.to_target_usize(tcx)),
                _ => None,
            }
        }
        // All other kinds (Param, Infer, etc.) → None
        _ => None,
    }
}

/// Evaluates a constant to target usize using the given parameter environment.
/// This function converts ParamEnv to TypingEnv internally and then evaluates the constant.
/// 
/// # Arguments
/// * `tcx` - The type context
/// * `param_env` - The parameter environment (may contain trait bounds and other constraints)
/// * `c` - The constant to evaluate
/// 
/// # Returns
/// * `Some(u64)` - If the constant can be evaluated to a target usize
/// * `None` - If the constant cannot be evaluated (e.g., contains generic parameters)
#[inline]
pub fn try_eval_target_usize_with_param_env<'tcx>(
    tcx: TyCtxt<'tcx>,
    param_env: ParamEnv<'tcx>,
    c: rustc_middle::ty::Const<'tcx>,
) -> Option<u64> {
    // Convert ParamEnv to TypingEnv for const evaluation
    // Using PostAnalysis mode, which is appropriate for evaluating constants after type checking
    let typing_env = TypingEnv {
        typing_mode: TypingMode::PostAnalysis,
        param_env,
    };
    try_eval_target_usize(tcx, typing_env, c)
}
