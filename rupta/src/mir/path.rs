...
/*
pub type Ty<'tcx> = TyS<'tcx>;
pub struct TyS<'tcx> {
    pub kind: TyKind<'tcx>,
    pub flags: TypeFlags,
    pub wrapper_span: Option<Span>,
    pub super_predicates: Option<ty::GenericPredicates<'tcx>>,
    ....
    }
pub enum TyKind<'tcx> {
    Adt(&'tcx AdtDef, &'tcx SubstsRef<'tcx>),
    Foreign(DefId),
    Str,
    Slice(Ty<'tcx>),
    ...
    FnDef(DefId, &'tcx SubstsRef<'tcx>),
    FnPtr(FnSig<'tcx>),
    Dynamic(&'tcx List<ExistentialPredicate<'tcx>>, Region<'tcx>),
    Closure(DefId, &'tcx SubstsRef<'tcx>),
    Generator(DefId, &'tcx SubstsRef<'tcx>, Movability),
    GeneratorWitness(DefId, &'tcx SubstsRef<'tcx>),
    ...
    Opaque(DefId, &'tcx SubstsRef<'tcx>),
    Foreign(DefId),
    Error,
 */
...
pub enum PathEnum {
    ...
    /// the promoted constant is defined inside the function identified by def_id.
    /// the constant can be found in a method level table of MIR bodies using the ordinal.
    PromotedConstant {
        def_id: DefId,
        ordinal: usize,
    },
    ...
    QualifiedPath {
        //path to a pointer
        base: Rc<Path>,
        ///a QualifiedPath represents a single specific path obtained by applying a sequence of projections to a base path.
    ...
    /// A promoted array of &str
    PromotedStrRefArray,

    /// A promoted array of ArgumentV1
...
pub enum PathSelector {
    ...
    /// base select by Discriminant gives a concrete value of the enum item
    Discriminant,

    /// Cast a path into another type.
    /// The most common cases are casting a transparent wrapper type into its inner type or
    /// casting a type into a transparent wrapper type via pointer casting.
    /// e.g. in the paper cast outer to inner and cast inner to u32
    /// you can search type of an concrete cast op with the index in castTypeCache.
    Cast(usize),
}
...
impl Path {
    ...
    pub fn new_qualified(base: Rc<Path>, projection: ProjectionElems) -> Rc<Path> {
        // base cannot be a qualified path
        // a QualifiedPath represents a single specific path obtained by applying a sequence of projections to a base path.
    ...
    /// metadata: the second part of a fat pointer, which stores the vtable pointer for trait objects
    ...
    /// add a single projection element to the given path
        ...
        match &path.value {
            ...
            _ => {
                if let PathEnum::QualifiedPath { base: _, projection } = &path.value {
                    // only allow the case where the path is a deref path
    ...
    pub fn truncate_projection_elems(path: &Rc<Path>, len: usize) -> Rc<Path> {
        if let PathEnum::QualifiedPath { base, projection } = &path.value {
            if projection.len() < len {
                warn!("The given length is longer than the projection elements length.");
                path.clone()
            } else {
                if len == 0 {
                    base.clone()
    ...
    /// cancel the last cast projection if exists
...
impl PathSupport for Rc<Path> {
    ...
    /// e.g. `_1.0.1` and `_1.0`
    /// field is longer than the given path
    fn is_field_of(&self, path: &Rc<Path>) -> bool {
    ...
    /// deref can only be base or the first projection element
    fn is_deref_path(&self) -> bool {
        match &self.value {
...
impl PAGPath for Rc<Path> {
    ...
    fn flatten_fields<'tcx>(self, acx: &mut AnalysisContext<'tcx, '_>) -> Vec<(usize, Self, Ty<'tcx>)> {
        ...
        type_util::flatten_fields(acx.tcx, param_env.param_env, self, path_ty)
    }
