...
/*
Maintain a hashmap between defid and enum elements of KnownNames.
implement functions to update and query the cache.
implement functions to get the known name for a given defid. (but it seems that the method should be used for many times?)

gramma std::slice::Iter::next() change the iterator state.
 */
impl KnownNamesCache {
    ...
    pub(crate) fn get_known_name_for(tcx: TyCtxt<'_>, def_id: DefId) -> KnownNames {
        ...
        fn foo(layout: std::alloc::Layout) {
            unsafe{ let _ptr = alloc(layout);}
        }

        tcx.crate_name(def_id.krate)  = "std"
        get_path_data_elem_name(def_path_data_iter.next()) = "alloc"
        then get_known_name_for_alloc_namespace(def_path_data_iter)
         */
