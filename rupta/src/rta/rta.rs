...
impl<'a, 'tcx, 'compilation> RapidTypeAnalysis<'a, 'tcx, 'compilation> {
    ...
    fn solve_fnptr_callsites(&mut self) {
        ...
        for (fnptr_type, callsites) in fnptr_callsites {
            /* #072 @lzq */
            // Rust nightly 2025-05-09: TyKind::FnPtr now carries (Binder<FnSigTys>, FnHeader)
            // Reconstruct a PolyFnSig from these two pieces for downstream matching logic.
            if let rustc_middle::ty::TyKind::FnPtr(binder_tys, header) = fnptr_type.kind() {
                let fn_sig: rustc_middle::ty::PolyFnSig<'tcx> = binder_tys.map_bound(|tys| rustc_middle::ty::FnSig {
    ...
    pub fn add_possible_fnptr_target(&mut self, fnptr_type: Ty<'tcx>, fn_item_type: Ty<'tcx>) {
        ...
        if let rustc_middle::ty::TyKind::FnPtr(binder_tys, header) = fnptr_type.kind() {
            ...
            self.fnptr_sig_to_possible_targets.entry(fn_sig).or_default().insert(fn_item_type);
