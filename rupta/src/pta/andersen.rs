...
impl<'pta, 'tcx, 'compilation> AndersenPTA<'pta, 'tcx, 'compilation> {
    ...
    ///author:wy
    ///dare:2025-11-04
    ///process the calls in the function pag
    ///for static and special calls, we add call edge to the call graph (not handle influcence in pfg)
    ///for dynamic and fnptr calls, we add them to the dynamic_calls and fnptr_calls maps.
    ...
    // author:wy
    // date:2025-10-30
    /// essentially, the two functions abstract the logic of adding new call edges and recursively call process_reach_funcs from ProcessCall of Taie
