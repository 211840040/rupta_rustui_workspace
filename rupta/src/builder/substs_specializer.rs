...
//! Author: wangyan
//! Date: 2025-10-24
//! cannot understand without enough rust code example and rustc knowledge
//! just know that it is used to specialize generic types to concrete types if a type may be generic
...
use rustc_middle::ty::{GenericArg, GenericArgKind, GenericArgsRef, TypingEnv};
...
pub struct SubstsSpecializer<'tcx> {
    ...
    // closures being specialized would be changed during specialization of their generic arguments, so RefCell is used here
...
impl<'tcx> SubstsSpecializer<'tcx> {
    ...
    Author:wangyan
    Date:2025-10-22
    ///mk_args creates a GenericArgsRef from a slice of GenericArg to be used in type-related operations.
    ///specialize_generic_args specializes each GenericArg in args to a concrete type or constant referring to self.generic_args.
    ///e.g. foo<T, U, const N: usize>(x: T, y: U, arr: [u8; N]) { ... }
    /// input: args = [T, U, N] output: specialized_generic_args = [i32, f64, 4]
    */
    ...
    pub fn specialize_generic_argument_type(&self, gen_arg_type: Ty<'tcx>) -> Ty<'tcx> {
        ...
        //     def_id: DefId,           // 关联类型定义
        //     args: GenericArgsRef<'tcx>, // 泛型参数（如 T, U, ...）
        // }
        //
        //  trait MyTrait<U> {
        //     type AssocType;
        // }
        //
        //  fn bar<T: MyTrait<u32>>(x: T) -> <T as MyTrait<u32>>::AssocType {
        //     ...
        //}
        // def_id points to AssocType, args is [T, u32]

        //param_env describes the environment of generic parameters for a given def_id. including bounds on type parameters....
        //associated_item gets the associated item (method, type, or constant) corresponding to the given def_id.
        //container_id gets the DefId of the item (trait or impl) that contains this
        if let TyKind::Alias(rustc_middle::ty::Projection, projection) = gen_arg_type.kind() {
            ...
            return if type_util::are_concrete(specialized_substs) {
                ...
                // construct specialized projection type
                ...
                // the case that normalization fails, e.g. trait has not been implemented for the concrete type
                // try to resolve the associated type via Instance resolution
                if let Ok(Some(instance)) = rustc_middle::ty::Instance::try_resolve(
                    ...
                    if item_def_id == instance_item_def_id {
                        ...
                        // fn call_fn_once<F: FnOnce(i32) -> i64>(f: F) -> <F as FnOnce(i32)>::Output {
                        //     f(42)
                        // }
                        // fn main() {
                        //     let result = call_fn_once(foo);
                        //     // 这里 F = fn(i32) -> i64，投影 <F as FnOnce(i32)>::Output
                        //     // specialized_substs[0] 是 FnDef
                        //     let result = call_fn_once(|x| x as i64 + 1);
                        //     // 这里 F 是 closure，投影 <F as FnOnce(i32)>::Output
                        //     // specialized_substs[0] 是 Closure
                        //     let fn_ptr: fn(i32) -> i64 = foo;
                        //     let result = call_fn_once(fn_ptr);
                        //     // 这里 F = fn(i32) -> i64（函数指针），投影 <F as FnOnce(i32)>::Output
                        //     // specialized_substs[0] 是 FnPtr
                        // }
        ...
        match gen_arg_type.kind() {
            ...
            TyKind::FnPtr(fn_sig_tys, header) => {
                let fn_sig = fn_sig_tys.with(*header);
                let specialized_fn_sig = fn_sig.map_bound(|fn_sig| {
