...
/// 
/// transform trait object calls to specific function calls.
...
pub fn try_to_devirtualize<'tcx>(
    ...
    // to a dynamic type.
    // so how to handle dynamic type calls?
    ...
    // let param_env = rustc_middle::ty::ParamEnv::reveal_all(); // old API (pre-2024-02-03)
    ...
    let abi = tcx
        ...
        .fn_sig(tcx)
        .abi();
    // if abi == rustc_target::spec::abi::Abi::Rust { // old API path (pre-2024-02-03)
    let resolved_instance = if abi == ExternAbi::Rust {
        // Instance::try_resolve returns an error if try_normalize_erasing_regions returns an error.
        // It is difficult to determine exactly when this error will occur.
        // if tcx.try_normalize_erasing_regions(param_env, gen_args).is_err() { // old signature
        if tcx.try_normalize_erasing_regions(typing_env, gen_args).is_err() {
            None
        } else {
            // Some(rustc_middle::ty::Instance::resolve(tcx, param_env, def_id, gen_args)) // old API
