...
This project’s VSCode configuration uses system environment variables to auto-locate the rust-analyzerpath. Please verify the following after cloning:

1. Find the local absolute path similar to `~/.rustup/toolchains/{toolchain-name}/bin/rust-analyzer` and paste it into the `"rust-analyzer.server.path"` configuration item in `".vscode/settings"`.
## Build
